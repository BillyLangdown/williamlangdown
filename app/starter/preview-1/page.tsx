...
const reviews = [
  { name: 'Sam Ellis', meta: 'Member since 2022', text: 'Best gym I’ve trained at. Coaches actually watch your form instead of just handing out programmes.' },
  { name: 'Priya Nair', meta: 'Member since 2021', text: 'No ego, no judgement, just people getting stronger together. Exactly what I needed.' },
  { name: 'Tom Whitfield', meta: 'Member since 2023', text: 'Small classes make a huge difference. You’re never just a number here.' },
]

function Stars({ className, size = 14 }: { className?: string; size?: number }) {
  return (
    <span className={`inline-flex gap-0.5 shrink-0 ${className ?? ''}`} style={{ color: '#FF5A1F' }} aria-hidden>
    ...
    <>
      ...
      <section ref={heroRef} className="relative px-6 pt-20 pb-24 md:pt-28 md:pb-32 overflow-hidden">
        ...
        <motion.div variants={heroContainer} initial="hidden" animate="visible" className="relative max-w-5xl mx-auto">
          <motion.p variants={heroItem} className="text-xs font-bold uppercase tracking-[0.3em] mb-6" style={{ color: '#FF5A1F' }}>
            Bristol · Est. 2019
          </motion.p>
          ...
          <motion.div variants={heroItem} className="mt-6 flex items-center gap-2">
            <Stars />
            <span className="text-sm text-white/70">4.9 star reviews from members</span>
          </motion.div>
        </motion.div>
      </section>
        ...
        >
          {[['6', 'Coaches'], ['150+', 'Members'], ['5', 'Years running']].map(([num, label]) => (
            <motion.div key={label} variants={gridItem}>
              <StatCounter value={num} label={label} numberColor="#FF5A1F" labelColor="rgba(255,255,255,0.5)" />
            </motion.div>
          ))}
        </motion.div>
      </section>
              ...
              {reviews.map(review => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0" style={{ background: 'rgba(255,90,31,0.15)', color: '#FF5A1F' }}>
      ...
      <section className="px-6 pb-24">
        <Reveal className="max-w-5xl mx-auto px-8 py-14 text-center" style={{ background: '#141414' }}>
          ...
          <GlowLink href="/starter/preview-1/contact">Get started</GlowLink>
        </Reveal>
      </section>
    </>
  )
  ...
  return (
    <Link href={href} className="relative inline-block mt-8">
      <motion.span
        ...
        animate={{ boxShadow: ['0 0 0px 0px rgba(255,90,31,0.45)', '0 0 32px 6px rgba(255,90,31,0.35)', '0 0 0px 0px rgba(255,90,31,0.45)'] }}
